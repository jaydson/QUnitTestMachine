...
var QUnitTestMachine = {
    ...
    loadScripts : function(){
        ...
        var elem = document.getElementById('qunit_test_machine_error');
            ...
            for(var i=0;i<scriptsLength;i++){
                $.ajax({
                    ...
                    success: function(){
                        ...
                        loadeds.innerHTML += '<span style="color:#5E740B"><b>Ok</b> - ' + path+scripts[i] + '</span><br>';
                        $.ajax({
                            ...
                            // Show which script was loaded
                            success : function(){
                                 loadeds.innerHTML += '<span style="color:#5E740B"><b>Ok</b> - ' + path+prefix+scripts[i] + '</span><br>';
                            },
                            
                            // Some trouble loading test file, show it
                            error: function(xhr){
                                var message = 'Error loading <a href="'+  path + prefix + scripts[i] +'">'+
                                               prefix + scripts[i] + '</a> (' + xhr.status + ' - ' + xhr.statusText +
                    ...
                    error: function(xhr){
                        var message = 'Error loading <a href="'+  path + scripts[i] +'">' +
                                       scripts[i] + '</a> (' + xhr.status + ' - ' + xhr.statusText + ')<br>';
                        elem.innerHTML+= message;
                        $('#qunit_test_machine').css('display','block');
                    }
                });
